    ...
    public class SearchEngineTestsBase
    {
        ...
        protected static void AssertSizeCounts(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
        {
            ...
            foreach (var size in Size.All)
            {
                ...
                // change - in the original query there was no filter with the size option
                // As per instructions only selected size should be counted?
                var expectedSizeCount = shirts
                    .Count(s => s.Size.Id == size.Id
                                && (!searchOptions.Sizes.Any() || searchOptions.Sizes.Select(sc => sc.Id).Contains(s.Size.Id))
                                && (!searchOptions.Colors.Any() || searchOptions.Colors.Select(cs => cs.Id).Contains(s.Color.Id)));
        ...
        protected static void AssertColorCounts(List<Shirt> shirts, SearchOptions searchOptions, List<ColorCount> colorCounts)
        {
            ...
            foreach (var color in Color.All)
            {
                ...
                // change - in the original query there was no filter with the color option
                // As per instructions only selected color should be counted?
                var expectedColorCount = shirts
                    .Count(c => c.Color.Id == color.Id  
                                && (!searchOptions.Colors.Any() || searchOptions.Colors.Select(cs => cs.Id).Contains(c.Color.Id))
                                && (!searchOptions.Sizes.Any() || searchOptions.Sizes.Select(sc => sc.Id).Contains(c.Size.Id)));
